...
`;
  ...
  function toVariantValue(variant) {
    if (variant === "round") return 1.0;
    if (variant === "snowflake") return 2.0;
    return 0.0;
  }

  function initPixelSnow(options = {}) {
    const THREE = window.THREE;
    if (!THREE) return;

    const ambient = document.querySelector(".ambient-bg");
    if (!ambient) return;
    if (ambient.__pixelSnowCleanup) ambient.__pixelSnowCleanup();

    const container = document.createElement("div");
    container.className = "pixel-snow-container";
    ambient.appendChild(container);
    ...
    const uniforms = {
      uTime: { value: 0 },
      uResolution: { value: new THREE.Vector2(container.offsetWidth || window.innerWidth, container.offsetHeight || window.innerHeight) },
      ...
      uVariant: { value: toVariantValue(options.variant || "square") },
    ...
    ambient.__pixelSnowCleanup = () => {
      ...
      delete ambient.__pixelSnowCleanup;
    };
  }
